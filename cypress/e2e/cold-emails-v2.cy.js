...
describe('Cold Emails V2 API Tests', () => {
    before(() => {
    ...
    environments().forEach((env) => {
        describe(`${env.toUpperCase()} Environment`, () => {
            ...
            context('Positive Test Cases @positive', () => {
                it('should successfully generate cold email with valid input', () => {
                    const apiUrl = `${config.baseUrl}/v2/business/content/cold-emails-v2`;
                ...
                it('should validate response time is within acceptable range', () => {
                    makeRequest({
                        method: 'POST',
                        url: `${config.baseUrl}/v2/business/content/cold-emails-v2`,
                        headers,
                        queryParams,
                        body: defaultPayload,
                        failOnStatusCode: false
            ...
            context('Negative Test Cases @negative', () => {
                it('should handle empty you field', () => {
                    makeRequest({
                        method: 'POST',
                        url: `${config.baseUrl}/v2/business/content/cold-emails-v2`,
                        headers,
                        queryParams,
                        body: { ...defaultPayload, you: "" },
                        failOnStatusCode: false
                    }).then((response) => {
                ...
                it('should handle empty goal field', () => {
                    makeRequest({
                        method: 'POST',
                        url: `${config.baseUrl}/v2/business/content/cold-emails-v2`,
                        headers,
                        queryParams,
                        body: { ...defaultPayload, goal: "" },
                        failOnStatusCode: false
                    }).then((response) => {
                        expect(response.status).to.eq(400);
                    });
                ...
                it('should handle empty scenario field', () => {
                    makeRequest({
                        method: 'POST',
                        url: `${config.baseUrl}/v2/business/content/cold-emails-v2`,
                        headers,
                        queryParams,
                        body: { ...defaultPayload, scenario: "" },
                        failOnStatusCode: false
                    }).then((response) => {
                        expect(response.status).to.eq(400);
                    });
