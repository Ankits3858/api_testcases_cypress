...
describe('Google Ad Descriptions API Tests', () => {
    before(() => {
    ...
    environments().forEach((env) => {
        describe(`${env.toUpperCase()} Environment`, () => {
            ...
            context('Positive Test Cases @positive', () => {
                it('should successfully generate google ad description with valid input', () => {
                    const apiUrl = `${config.baseUrl}/v2/business/content/google-ad-descriptions`;
                ...
                it('should validate response time is within acceptable range', () => {
                    makeRequest({
                        method: 'POST',
                        url: `${config.baseUrl}/v2/business/content/google-ad-descriptions`,
                        headers,
                        queryParams,
                        body: defaultPayload,
                        failOnStatusCode: false
            ...
            context('Negative Test Cases @negative', () => {
                ...
                it('should handle empty primary_keyword field', () => {
                    makeRequest({
                        method: 'POST',
                        url: `${config.baseUrl}/v2/business/content/google-ad-descriptions`,
                        headers,
                        queryParams,
                        body: { ...defaultPayload, primary_keyword: "" },
                        failOnStatusCode: false
                    }).then((response) => {
                        expect(response.status).to.eq(400);
                    });
                ...
                it('should handle empty secondary_keywords field', () => {
                    makeRequest({
                        method: 'POST',
                        url: `${config.baseUrl}/v2/business/content/google-ad-descriptions`,
                        headers,
                        queryParams,
                        body: { ...defaultPayload, secondary_keywords: "" },
                        failOnStatusCode: false
                    }).then((response) => {
                        expect(response.status).to.eq(400);
                    });
