...
describe('YouTube Intros V2 API Tests', () => {
    before(() => {
    ...
    environments().forEach((env) => {
        describe(`${env.toUpperCase()} Environment`, () => {
            ...
            const defaultPayload = {
                video_title: "Launch of Writesonics chrome extension that lets anyone rephrase, expand and shorten their content within seconds.",
            ...
            context('Positive Test Cases @positive', () => {
                it('should successfully generate youtube intro v2 with valid input', () => {
                    const apiUrl = `${config.baseUrl}/v2/business/content/youtube-intros-v2`;
                ...
                it('should validate response time is within acceptable range', () => {
                    makeRequest({
                        method: 'POST',
                        url: `${config.baseUrl}/v2/business/content/youtube-intros-v2`,
                        headers,
                        queryParams,
                        body: defaultPayload,
                        failOnStatusCode: false
            ...
            context('Negative Test Cases @negative', () => {
                it('should handle empty video_title field', () => {
                    makeRequest({
                        method: 'POST',
                        url: `${config.baseUrl}/v2/business/content/youtube-intros-v2`,
                        headers,
                        queryParams,
                        body: { ...defaultPayload, video_title: "" },
                        failOnStatusCode: false
                    }).then((response) => {
                ...
                it('should handle empty hook field', () => {
                    makeRequest({
                        method: 'POST',
                        url: `${config.baseUrl}/v2/business/content/youtube-intros-v2`,
                        headers,
                        queryParams,
                        body: { ...defaultPayload, hook: "" },
                        failOnStatusCode: false
                    }).then((response) => {
                        expect(response.status).to.eq(400);
                    });
                ...
                it('should handle empty tone field', () => {
                    makeRequest({
                        method: 'POST',
                        url: `${config.baseUrl}/v2/business/content/youtube-intros-v2`,
                        headers,
                        queryParams,
                        body: { ...defaultPayload, tone: "" },
                        failOnStatusCode: false
                    }).then((response) => {
                        expect(response.status).to.eq(400);
                    });
