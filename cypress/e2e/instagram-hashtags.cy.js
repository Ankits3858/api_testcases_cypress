...
describe('Instagram Hashtags API Tests', () => {
    before(() => {
    ...
    environments().forEach((env) => {
        describe(`${env.toUpperCase()} Environment`, () => {
            ...
            context('Positive Test Cases @positive', () => {
                it('should successfully generate instagram hashtags with valid input', () => {
                    const apiUrl = `${config.baseUrl}/v2/business/content/instagram-hashtags`;
                    
                    makeRequest({
                        method: 'POST',
                        url: apiUrl,
                        headers,
                        queryParams: { ...queryParams, engine: 'premium' },
                        body: defaultPayload,
                ...
                it('should validate response time is within acceptable range', () => {
                    makeRequest({
                        method: 'POST',
                        url: `${config.baseUrl}/v2/business/content/instagram-hashtags`,
                        headers,
                        queryParams: { ...queryParams, engine: 'premium' },
                        body: defaultPayload,
                        failOnStatusCode: false
            ...
            context('Negative Test Cases @negative', () => {
                ...
                it('should handle empty language parameter', () => {
                    makeRequest({
                        ...
                        url: `${config.baseUrl}/v2/business/content/instagram-hashtags`,
                        headers,
                        queryParams: { ...queryParams, engine: 'premium', language: '' },
                        body: defaultPayload,
                        failOnStatusCode: false
                    }).then((response) => {
                        expect(response.status).to.eq(400);
                ...
                it('should handle empty num_copies parameter', () => {
                    makeRequest({
                        ...
                        url: `${config.baseUrl}/v2/business/content/instagram-hashtags`,
                        headers,
                        queryParams: { ...queryParams, engine: 'premium', num_copies: '' },
                        body: defaultPayload,
                        failOnStatusCode: false
                    }).then((response) => {
                        expect(response.status).to.eq(400);
